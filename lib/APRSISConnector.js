...
APRSISConnector.prototype.connect = function (callsign, options) {
    if (this.client) {
        this.disconnect();
    }
    ...
    that.client.connect(options.port || APRSPort, options.server || APRSServer, function () {
        var connectString = "user " + callsign;
        if (options.pass) {
            connectString += " pass " + options.pass;
        }
        if (options.filter) {
            connectString += " filter " + options.filter;
        }
        connectString += "\n";
        that.client.write(connectString);
        that.emit("connected", connectString);
    });
